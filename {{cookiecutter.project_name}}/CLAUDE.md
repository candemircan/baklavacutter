...
### Setup
```bash
bash setup.sh              # Install uv, sync deps, install pre-commit hooks
```
Always use `uv` for package management (never `pip`). Run scripts with `uv run`.

### Running Tests
```bash
bash bin/run_tests.sh {{cookiecutter.project_name}}/*.py  # Run all module tests
uv run -m {{cookiecutter.project_name}}.module_name       # Single module test
```
Each core module has a `@call_parse def main()` test harness in its `__main__` block.

### Linting
Pre-commit hooks run automatically: `ruff` (unused imports + isort) and tests on `{{cookiecutter.project_name}}/*.py` files.
...
- **`__all__`**: Define `__all__ = [...]` in every module to explicitly declare the public API.
